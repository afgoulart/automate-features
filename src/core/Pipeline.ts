...
export class Pipeline {
  ...
  constructor(config: PipelineConfig) {
    ...
    aiProviderTypeStr = aiProviderTypeStr
      .toUpperCase()
      .replace('CLOUD_CODE', 'CLAUDE_CODE')
      .replace('CLOUD', 'CLAUDE')
      .replace('COURSOR', 'CURSOR');

    const aiProviderType = (aiProviderTypeStr === 'CLAUDE_CODE' ? 'CLAUDE_CODE' : 'CURSOR') as
      | 'CURSOR'
      | 'CLAUDE_CODE';
    ...
    console.log(
      `[Pipeline] Creating CodeGenerator with useCli=${useCli}, sourceDir=${sourceDir}, aiProviderType=${this.config.aiProviderType}`
    );
